...
RELEASE_NAMESPACE="${RELEASE_NAMESPACE:-${TARGET_RELEASE_NAMESPACE:-${ZARF_HELM_RELEASE_NAMESPACE:-${HELM_RELEASE_NAMESPACE:-default}}}}"
RELEASE_NAME_SOURCE="${RELEASE_NAME_SOURCE:-${TARGET_RELEASE_SOURCE:-${HELM_RELEASE_SOURCE:-}}}"
RELEASE_NAME="${RELEASE_NAME:-${TARGET_RELEASE_NAME:-${ZARF_HELM_RELEASE_NAME:-${HELM_RELEASE_NAME:-}}}}"
...
detect_release_metadata() {
  ...
  if [[ "${RELEASE_SUPPLIED}" == true && -n "${RELEASE_NAMESPACE}" ]]; then
  ...
  RELEASE_NAME="$(kubectl get crd "${probe_crd}" -o jsonpath='{.metadata.annotations.meta\.helm\.sh/release-name}' || true)"
  RELEASE_NAMESPACE="$(kubectl get crd "${probe_crd}" -o jsonpath='{.metadata.annotations.meta\.helm\.sh/release-namespace}' || true)"
